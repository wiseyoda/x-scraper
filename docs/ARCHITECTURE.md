...
| Graph DB              | **Kùzu** (embedded, native Node bindings, Cypher, HNSW)                                    | Single binary, no daemon, vector index built-in, MIT, used by Graphiti.                                                           |
...
│   ├── graph/            # Kùzu adapter, schema migrations, ER, contradiction detection
